...
def main() -> None:
    ...
    parser.add_argument("--cold-start", choices=("zero", "baseline"), required=True)
    parser.add_argument("--output-dir", type=Path)
    args = parser.parse_args()
    paths = attachment_paths(ROOT, args.attachment_dir)
    output_dir = args.output_dir or (ROOT / "output" / "model_versions" / f"q2_causal_{args.cold_start}")
    ...
    write_outputs(output_dir.resolve(), paths["template2"], dates, price, records, summary)
