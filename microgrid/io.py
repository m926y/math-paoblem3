...
from .timegrid import validate_end_labels, interval
...
def _read_daily_sheet(sheet, label: str) -> tuple[list[date], np.ndarray]:
    validate_end_labels(next(sheet.iter_rows(min_row=1,max_row=1,min_col=2,max_col=T+1,values_only=True)))
...
def write_result2(template: Path, output: Path, dates: list[date], price: np.ndarray,
    ...
    for row_no, (date_value, record) in enumerate(zip(dates, records), start=2):
        ...
        if question == 3:
            ...
            fee=settle(price,record['grid'],record['emergency'],record['grid0'])
